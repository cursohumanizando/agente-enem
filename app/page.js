        ...
        {q.recursoVisual && q.recursoVisual.descricao && (() => {
          ...
          return (
            <div style={{ padding:"14px 16px", background:"rgba(255,209,102,.07)", border:"1.5px dashed #ffd16688", borderRadius:"10px", marginBottom:"18px" }}>
              <div style={{ display:"flex", gap:"10px", alignItems:"center", marginBottom: wimg ? "10px" : "6px" }}>
                ...
                <span style={{ fontFamily:"'DM Mono',monospace", fontSize:"10px", color:"#ffd166", letterSpacing:"2px", textTransform:"uppercase" }}>
                  {q.recursoVisual.tipo}{wimg ? " — Wikimedia Commons" : " sugerido"}
                </span>
              </div>
              {wimg ? (
                <img src={wimg.dataUrl} alt={q.recursoVisual.descricao}
                  style={{ width:"100%", maxHeight:"320px", objectFit:"contain", borderRadius:"8px", display:"block", marginBottom:"8px" }} />
              ) : (
              ...
              {q.recursoVisual.fonteRecurso && (
                <div style={{ fontSize:"11px", color:"var(--muted)", fontFamily:"'DM Mono',monospace" }}>
                  📎 Fonte sugerida: {q.recursoVisual.fonteRecurso}
                </div>
              )}
              {wimg && (
...
export default function App() {
  ...
  // Imagens Wikimedia (indexed by questao numero)
  ...
  // ── Busca imagens no Wikimedia Commons para questões com recurso visual ──
  async function buscarImagensWikimedia(qs) {
    ...
    await Promise.all(comVisual.map(async (q) => {
      ...
      } catch (_) { /* silencioso — imagem não essencial */ }
    }));
  ...
  async function exportarPDF(qs, nomeArq) {
    const res = await fetch("/api/pdf", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ questoes: qs, wikiImgs }),
    });
  ...
  async function gerarNoEstilo() {
    ...
    function buildPromptLote(offset, qtdLote) {
      ...
      return "Você é elaborador ENEM. Gere EXATAMENTE " + qtdLote + " questão(ões) novas no estilo abaixo.\n\n" +
        ...
        "2. GEOGRAFIA: diferencie SEMPRE entre Humana (urbanização, migração, geopolítica, demografia, desigualdades) e Física (clima, relevo, biomas, hidrografia, impactos ambientais). Distribua equilibradamente.\n" +
        "3. Texto-base suficiente e necessário (5–12 linhas conforme nível).\n" +
        "4. FORMATO DO COMANDO — REGRA MAIS IMPORTANTE:\n" +
        "   O comando NÃO é uma pergunta completa. É uma FRASE NOMINAL INCOMPLETA que o candidato completa ao escolher a alternativa.\n" +
        "   A frase termina em artigo (a/o/as/os) ou preposição (da/do/pelo/pela/para) — NUNCA com ponto após verbo conjugado.\n\n" +
        "   EXEMPLOS REAIS DO ENEM 2025 — use esta estrutura:\n" +
        "   • \"A [ação] descrita no texto origina-se da\"\n" +
        "   • \"A crítica do texto evidencia uma dinâmica socioespacial marcada pela\"\n" +
        "   • \"Os argumentos do texto sustentam que [tema] porque os considera como\"\n" +
        "   • \"A [mudança] descrita provocou a diminuição da\"\n" +
        "   • \"A [ação] dos [sujeito] teve como consequência imediata o(a)\"\n" +
        "   • \"De acordo com o texto, a [variação] foi determinada pelo(a)\"\n" +
        "   • \"No contexto de [período], o autor propõe uma reflexão, restringindo-a ao\"\n" +
        "   • \"Para [sujeito], a mudança de [elemento] é uma estratégia para\"\n" +
        "   • \"Publicados em períodos diferentes, os textos apresentam\"\n\n" +
        "   PERGUNTA DIRETA: permitida apenas 1 a cada 5 questões.\n" +
        "   Ex: \"Qual condição favoreceu o cenário exposto?\" / \"Qual é o efeito do problema no texto?\"\n\n" +
        "   ABSOLUTAMENTE PROIBIDO: imperativo (Identifique/Analise/Compare/Explique) | segundo o texto | de acordo com o texto | com base no texto\n" +
        "5. Alternativas nominais paralelas, 6–20 palavras, sem verbos conjugados.\n" +
        "6. Distratores com desvios distintos: generalização | redução | inversão de causalidade | deslocamento temporal | confusão conceitual\n" +
        "7. Fonte real com autor e ano.\n" +
        "8. Gabarito EXATAMENTE a letra da lista.\n" +
        "9. Campo 'explicacao' de cada opção: 1–2 frases explicando por que está certa ou errada.\n" +
        "10. RECURSO VISUAL: pelo menos 1 a cada 3 questões deve ter recursoVisual (tipo, descricao, fonteRecurso com URL real). Questões com recurso visual podem ter textoBase vazio — a imagem é o suporte.\n" +
        "11. DOIS TEXTOS: pelo menos 1 a cada 4 questões (Médio/Difícil) deve usar Texto I + Texto II.\n" +
        "    Formato no textoBase: \"TEXTO I\\n[texto]\\n\\nTEXTO II\\n[texto]\". Comando: \"Publicados em períodos diferentes, os textos apresentam\"\n\n" +
        "Responda SOMENTE JSON válido sem markdown:\n" +
