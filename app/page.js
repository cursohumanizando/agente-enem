...
function buildPrompt({ disciplina, niveis, tipoItem, tema, qtd }) {
  const dist = buildDistribuicao(qtd, niveis);
  const letras = ["A","B","C","D","E"];
  const gabs = Array.from({ length: qtd }, (_, i) => letras[i % 5]);
  const lista = dist.map((n, i) => `Q${i+1}: ${n} — gabarito: ${gabs[i]}`).join("\n");


  return `Você é um elaborador especialista de itens no padrão ENEM, Ciências Humanas e suas Tecnologias.

═══ ESPECIFICAÇÃO DO ITEM ═══
Disciplina: ${disciplina}
Tipo de item: ${tipoItem}
Tema/Contexto: ${tema || "livre, adequado à disciplina"}
Quantidade: ${qtd} questão(ões)

HABILIDADE: você deve escolher a habilidade mais adequada da Matriz de Referência ENEM (H1–H30) de Ciências Humanas, com base na disciplina e no tema. Escolha habilidades distintas para cada questão quando houver mais de uma.

HABILIDADES DISPONÍVEIS PARA ${disciplina}:
...
🟢 FÁCIL — COMPREENSÃO (eixo DL/CF)
  • O candidato identifica ou reconhece informação explícita no texto-base
  • Verbo do comando: evidencia / demonstra / caracteriza / tem como objetivo / ilustra
  • Texto-base: 5–7 linhas, linguagem direta, 1 fonte
  • Distratores: desvios óbvios (inversão direta, generalização evidente)

🟡 MÉDIO — ANÁLISE (eixo SP/CA)
  • O candidato relaciona dois elementos ou articula conceito ao contexto apresentado
  • Verbo do comando: permite reconhecer / contribui para / encontra fundamento / reflete
  • Texto-base: 7–10 linhas, dado ou argumento a ser interpretado
  • Distratores: plausíveis, eliminados por leitura cuidadosa

🔴 DIFÍCIL — SÍNTESE/AVALIAÇÃO (eixo CA/EP)
  • O candidato articula dois textos, avalia contradição ou mobiliza conceito não explícito
  • Verbo do comando: permite inferir / revela a tensão entre / fundamenta-se na / pressupõe
  • Texto-base: 10–12 linhas OU dois textos complementares em diálogo
  • Distratores: sofisticados, verdadeiros em parte, eliminados só por síntese completa

═══ REGRAS OBRIGATÓRIAS ═══
1. Texto-base suficiente e necessário — sem ele a questão NÃO pode ser respondida
2. PROIBIDO no comando: "segundo o texto" / "de acordo com o texto" / "com base no texto" / "a partir do texto"
3. Alternativas com estrutura nominal paralela, 6–20 palavras cada
4. Cada distrator usa exatamente um tipo de desvio distinto:
   generalização indevida | redução indevida | inversão de causalidade |
   deslocamento temporal | confusão conceitual | relação parcial
5. Fonte real com autor e ano (ex: SANTOS, Milton. A Natureza do Espaço. 2002.)
6. O GABARITO de cada questão DEVE ser EXATAMENTE a letra indicada na lista acima
7. O campo "nivel" DEVE corresponder ao nível da lista acima
8. O campo "habilidade" deve conter o código oficial (ex: "H8") e seu enunciado completo

Responda SOMENTE com JSON válido, sem markdown, sem texto antes ou depois:
{"questoes":[{"tema":"","nivel":"Fácil|Médio|Difícil","textoBase":"","fonte":"","comando":"","opcoes":[{"letra":"A","texto":"","correta":false,"explicacao":""},{"letra":"B","texto":"","correta":false,"explicacao":""},{"letra":"C","texto":"","correta":false,"explicacao":""},{"letra":"D","texto":"","correta":false,"explicacao":""},{"letra":"E","texto":"","correta":false,"explicacao":""}],"gabarito":"","habilidade":"","competencia":"","eixo":""}]}`;
}
...
export default function App() {
  ...
  const [tipoItem, setTipo]       = useState("Interpretação de texto");
  ...
  async function exportarDOCX(qs, nomeArq) {
    ...
    const children = [
      p([t("SIMULADO ENEM · CIÊNCIAS HUMANAS", { bold:true, size:28, color:"6C63FF" })],
    ...
    qs.forEach((q, i) => {
      const hab = q.habilidade?.match(/H\d+/)?.[0] || "";
      children.push(p([
        t(`QUESTÃO ${i+1}`, { bold:true, size:22, color:"6C63FF" }),
        t(`   ·   ${(q.nivel||"").toUpperCase()}   ·   ${hab}`, { size:18, color:"9090C0" }),
  ...
  async function gerarQuestoes() {
    ...
    // ── 1. Gerar questões via Claude API ──
    let qs;
    try {
      setLoadingMsg("Gerando questões...");
      const res = await fetch("/api/gerar", {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 8000,
          messages: [{ role: "user", content: buildPrompt({ disciplina, niveis, tipoItem, tema, qtd }) }],
        }),
      });
      ...
      qs = parsed.questoes.map((q, i) => ({ ...q, numero: i+1, area: disciplina }));
    } catch(e) {
    ...
    // ── 2. Questão avulsa → exibe na tela ──
    if (!isSimulado) {
      setQuestoes(qs);
      setLoadingMsg("");
      setLoading(false);
    ...
    // ── 3. Simulado → sempre guarda as questões e mostra o painel de download ──
    ...
    if (errosExport.length > 0) {
      setErro(`Não foi possível gerar: ${errosExport.join(", ")}. Use o download TXT abaixo.`);
    }
  }
  ...
  async function analisarSimulado() {
    ...
    try {
      ...
      setErroImport("Não foi possível analisar o arquivo. Verifique se é um PDF ou Word legível.");
      console.error(e);
    } finally {
      setLoadImport(false);
    }
  ...
  // ── Gerar questões no estilo do simulado importado ──
  async function gerarNoEstilo() {
    if (!analise) return;
    setErro(""); setLoading(true); setQuestoes([]); setResps({}); setRevs({});

    const dist = buildDistribuicao(qtdImport, ["Fácil","Médio","Difícil"]);
    const letras = ["A","B","C","D","E"];
    const gabs = Array.from({ length: qtdImport }, (_, i) => letras[i % 5]);
    const lista = dist.map((n, i) => `Q${i+1}: ${n} — gabarito: ${gabs[i]}`).join("\n");

    const promptEstilo = `Você é um elaborador especialista de itens no padrão ENEM, Ciências Humanas.

Analise o estilo abaixo e gere ${qtdImport} questões NOVAS seguindo fielmente as mesmas características.

═══ ANÁLISE DO SIMULADO DE REFERÊNCIA ═══
Disciplinas identificadas: ${analise.disciplinas?.join(", ")}
Habilidades predominantes: ${analise.habilidades?.join(", ")}
Tipos de texto-base: ${analise.tiposTexto?.join(", ")}
Padrão dos comandos: ${analise.padraoComandos}
Complexidade dos distratores: ${analise.complexidadeDistratores}
Temas recorrentes: ${analise.temas?.join(", ")}
Observações de estilo: ${analise.observacoesEstilo}
...
2. PROIBIDO no comando: "segundo o texto" / "de acordo com o texto" / "com base no texto"
3. Alternativas nominais paralelas, 6–20 palavras
4. Distratores com desvios distintos
5. Fonte real com autor e ano
6. Gabarito EXATAMENTE a letra indicada
7. Escolha a habilidade ENEM mais adequada (H1–H30) para cada questão
    ...
    try {
      const res = await fetch("/api/gerar", {
        ...
        body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: 8000,
          messages: [{ role: "user", content: promptEstilo }] })
      });
      const data = await res.json();
      const txt = data.content?.map(b => b.type === "text" ? b.text : "").join("") || "";
      const parsed = JSON.parse(txt.replace(/```json|```/g,"").trim());
      const disc = analise.disciplinas?.[0] || "Geografia";
      setQuestoes(parsed.questoes.map((q, i) => ({ ...q, numero: i+1, area: disc })));
      setModo("avulsa"); // volta para a view de questões
    } catch(e) {
      setErro("Erro ao gerar questões no estilo importado.");
      console.error(e);
    } finally {
      setLoading(false);
    }
                  ...
                  <div className="fade-up" style={{ background:"var(--panel)", border:"1.5px solid #ffd16644", borderRadius:"12px", padding:"18px", marginBottom:"20px" }}>
                    ...
                    <div style={{ marginTop:"20px", paddingTop:"16px", borderTop:"1px solid var(--border)", display:"flex", alignItems:"center", gap:"12px", flexWrap:"wrap" }}>
                      ...
                      <button className="btn-gerar" onClick={gerarNoEstilo} disabled={loading}
                        ...
                        {loading ? <><span className="spin" style={{ display:"inline-block" }}>⟳</span> Gerando...</>
              ...
              <div className="fade-up" style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:"16px" }}>
                ...
                <div style={{ display:"flex", gap:"10px", flexWrap:"wrap", justifyContent:"center" }}>
                  ...
                  <button className="btn-ghost" style={{ fontSize:"13px", padding:"11px 18px" }}
                    onClick={() => {
                      const txt = questoes.map(q => `QUESTÃO ${q.numero}
${q.textoBase}
${q.fonte}
          ...
          <div className="fade-up" style={{ background:"var(--surface)", border:"1.5px solid var(--border)", borderRadius:"12px", padding:"14px 20px", marginBottom:"20px", display:"flex", alignItems:"center", gap:"16px" }}>
            ...
            <div style={{ display:"flex", gap:"6px" }}>
              <button className="btn-ghost" onClick={() => {
                const txt = questoes.map(q=>`QUESTÃO ${q.numero}\n${q.textoBase}\n${q.fonte}\n\n${q.comando}\n\n${q.opcoes.map(o=>`${o.letra}) ${o.texto}`).join("\n")}\n\nGabarito: ${q.gabarito}\n${"─".repeat(44)}`).join("\n\n");
                const b=new Blob([txt],{type:"text/plain"});const u=URL.createObjectURL(b);const a=document.createElement("a");a.href=u;a.download="simulado-enem.txt";a.click();URL.revokeObjectURL(u);
              }}>⬇ TXT</button>
              <button className="btn-ghost" onClick={() => {
