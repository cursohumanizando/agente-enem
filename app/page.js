...
function buildPrompt({ disciplina, niveis, tipoItem, tema, qtd, offset = 0 }) {
  ...
  return "Você é elaborador ENEM especialista em Ciências Humanas.\n\n" +
    ...
    "🟢 FÁCIL — COMPREENSÃO: comando afirmativo com verbos evidencia/demonstra/caracteriza/tem como objetivo/ilustra. Ex: \"O texto evidencia que...\" Texto 5–7 linhas, distratores óbvios.\n" +
    "🟡 MÉDIO — ANÁLISE: comando afirmativo com verbos permite reconhecer/contribui para/encontra fundamento/reflete. Ex: \"O gráfico permite reconhecer que...\" Texto 7–10 linhas, distratores plausíveis.\n" +
    "🔴 DIFÍCIL — SÍNTESE: comando afirmativo com verbos permite inferir/revela a tensão entre/fundamenta-se na/pressupõe. Ex: \"A comparação entre os mapas permite inferir que...\" Texto 10–12 linhas ou dois textos, distratores sofisticados.\n\n" +
    "═══ REGRAS OBRIGATÓRIAS ═══\n" +
    "1. DISCIPLINA E EIXO:\n" +"   - Se a disciplina for GEOGRAFIA, diferencie obrigatoriamente entre:\n" +"     • GEOGRAFIA HUMANA: urbanização, migração, geopolítica, fronteiras, demografia, territorialidade, trabalho, globalização, desigualdades socioespaciais.\n" +"     • GEOGRAFIA FÍSICA: clima, biomas, relevo, hidrografia, solos, intemperismo, dinâmica atmosférica, recursos naturais, impactos ambientais.\n" +"   - A habilidade mobilizada deve corresponder ao eixo correto (H6/H26/H29 = física; H7/H8/H9/H17/H18/H19 = humana).\n" +"   - Distribua equilibradamente entre os dois eixos quando houver múltiplas questões de Geografia.\n" +"1b. Texto-base suficiente e necessário\n" +
    "2. FORMATO DO COMANDO — padrão ENEM real:\n" +"   O comando é sempre uma frase afirmativa que termina indicando o que o candidato deve reconhecer/inferir. NUNCA imperativo.\n" +"   MODELOS REAIS DO ENEM (use exatamente esse tipo de estrutura):\n" +"   • \"Ao abordar [tema], esse texto tem por objetivo...\"\n" +"   • \"Nesse texto, ao discorrer sobre [tema], o autor tem por objetivo...\"\n" +"   • \"O elemento que caracteriza esse texto como [gênero] é a/o...\"\n" +"   • \"Nesse texto, o que caracteriza [elemento] é...\"\n" +"   • \"A autora conclui que [tema]...\"\n" +"   • \"De acordo com o texto, o programa voltado a [tema] evidencia a...\"\n" +"   • \"Nesse fragmento, o narrador/eu lírico/autor [ação] ao...\"\n" +"   • \"A [figura/charge/tabela/gráfico] permite identificar qual característica de...\"\n" +"   • \"Qual [condição/elemento/característica] [verbo] o cenário exposto?...\"\n" +"   • \"A dispersão espacial do problema representado é explicada pela...\"\n" +"   • \"O texto indica o seguinte aspecto de [tema] como...\"\n" +"   • \"No que diz respeito a [tema], o autor/a autora...\"\n" +"   PROIBIDO: \"Identifique\", \"Analise\", \"Compare\", \"Explique\", \"segundo o texto\", \"de acordo com o texto\", \"com base no texto\"\n" +
    "2b. PROIBIDO no comando: \"segundo o texto\" / \"de acordo com o texto\" / \"com base no texto\" / \"com base na imagem\"\n" +
    ...
    "8. RECURSO VISUAL: pelo menos 1 a cada 3 questões DEVE ter recursoVisual preenchido (tipo, descricao detalhada, fonteRecurso com URL real).\n" +"   IMPORTANTE: questões com recurso visual NÃO precisam obrigatoriamente de textoBase — podem ter APENAS a imagem/gráfico/mapa como suporte, como ocorre frequentemente no ENEM real (ex: Q47, Q63, Q68).\n" +"   Nesses casos, deixe textoBase vazio (\"\") e fonte com a referência da imagem.\n" +"   Questões de nível Médio e Difícil devem preferencialmente ter recurso visual.\n\n" +"9. QUESTÕES COM DOIS TEXTOS (TEXTO I e TEXTO II): pelo menos 1 a cada 4 questões de nível Médio ou Difícil deve apresentar dois textos complementares ou contraditórios como suporte.\n" +"   Formato obrigatório no campo textoBase: \"TEXTO I\\n[texto 1]\\n\\nTEXTO II\\n[texto 2]\"\n" +"   Modelos de uso no ENEM real: dois autores sobre o mesmo tema (Q53, Q85), texto + dado estatístico (Q49), fragmento literário + crítica (Q23).\n" +"   O comando deve articular os dois textos: \"A comparação entre os textos evidencia que...\" / \"Os textos I e II demonstram...\" / \"Publicados em períodos diferentes, os textos apresentam...\"\n\n" +
...
export default function App() {
  ...
  async function lerArquivo(file) {
    ...
    if (tipo === "pdf") {
      // PDF: extrai texto com PDF.js para evitar limite de tamanho
      setLoadImport(true);
      try {
        ...
        setArquivo({ nome, tipo: "docx", texto: textoTotal }); // trata como texto
      } catch(e) {
  ...
  async function gerarNoEstilo() {
    ...
    function buildPromptLote(offset, qtdLote) {
      ...
      return "Você é elaborador ENEM. Gere EXATAMENTE " + qtdLote + " questão(ões) novas no estilo abaixo.\n\n" +
        ...
        "REGRAS:\n" +
        "1. Cada questão deve abordar o tema especificado acima — NÃO repita temas\n" +"1b. GEOGRAFIA: diferencie obrigatoriamente entre Geografia Humana (urbanização, geopolítica, migração, trabalho, globalização, desigualdades) e Geografia Física (clima, relevo, biomas, hidrografia, intemperismo, recursos naturais). A habilidade deve corresponder ao eixo correto.\n" +
        "2. Texto-base necessário e suficiente\n" +
        "3. COMANDO — padrão ENEM real: frase afirmativa, NUNCA imperativo.\n" +"   Modelos: \"Ao abordar [tema], esse texto tem por objetivo...\" / \"Nesse texto, o autor tem por objetivo...\" / \"O gráfico/mapa/tabela permite identificar qual característica...\" / \"A análise evidencia que...\" / \"O texto indica o seguinte aspecto...\" / \"Qual [condição] favoreceu o cenário exposto?\"\n" +"   PROIBIDO: Identifique / Analise / Compare / Explique / segundo o texto / de acordo com o texto / com base no texto\n" +
        ...
        "9. RECURSO VISUAL: pelo menos 1 a cada 3 questões DEVE ter recursoVisual. Questões com recurso visual NÃO precisam de textoBase — podem ter APENAS imagem/gráfico/mapa como suporte (textoBase vazio \"\" nesses casos). Preencha tipo, descricao detalhada e fonteRecurso com URL real.\n" +"10. QUESTÕES COM DOIS TEXTOS: pelo menos 1 a cada 4 questões deve usar dois textos. Formato no textoBase: \"TEXTO I\\n[texto]\\n\\nTEXTO II\\n[texto]\". Comando: \"Os textos I e II demonstram...\" / \"A comparação entre os textos evidencia...\"\n\n" +
        "Responda SOMENTE JSON válido sem markdown:\n" +
