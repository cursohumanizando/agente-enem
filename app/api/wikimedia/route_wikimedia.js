...
// Biblioteca curada de recursos visuais
const RECURSOS = [
  // Mapas Brasil
  ...
  { tipo:['charge','fotografia'], keywords:['revolucao industrial','operario','fabrica','capitalismo industrial','chamine'], url:'https://upload.wikimedia.org/wikipedia/commons/thumb/a/a8/Uss_men_in_factory.jpg/800px-Uss_men_in_factory.jpg', fonte:'Wikimedia Commons' },
...
// Gera chave única para o Blob baseada na URL da imagem
function gerarChaveBlob(url) {
  const hash = url.split('/').pop().replace(/[^a-zA-Z0-9._-]/g, '_').slice(0, 60);
  return `enem-imagens/${hash}`;
}

// Baixa imagem e salva no Vercel Blob — usa cache para não baixar 2x
async function obterImagemBlob(sourceUrl, fonte) {
  const chave = gerarChaveBlob(sourceUrl);

  // Verifica se já está no Blob (cache)
  try {
    const existing = await head(chave);
    if (existing?.url) {
      return { found: true, imageUrl: existing.url, fonte };
    }
  } catch {
    // Não existe ainda, continua para baixar
  }

  // Baixa da fonte original
  ...
  const blob = await put(chave, Buffer.from(arrayBuffer), {
    access: 'public',
    contentType: mime,
    addRandomSuffix: false,
  });

  return { found: true, imageUrl: blob.url, fonte };
}
...
export async function POST(request) {
  try {
    ...
    // 1. Biblioteca curada → baixa e salva no Blob
    const recurso = encontrarRecurso(descricao, tipo, tema);
    if (recurso) {
      const result = await obterImagemBlob(recurso.url, recurso.fonte);
      if (result) return Response.json(result);
    }

    // 2. Fallback: busca no Wikimedia → salva no Blob
    const searchTerm = await gerarTermoBusca(descricao, tipo);
    if (searchTerm) {
      const wikimediaUrl = await buscarUrlWikimedia(searchTerm);
      if (wikimediaUrl) {
        const result = await obterImagemBlob(wikimediaUrl, 'Wikimedia Commons');
        if (result) return Response.json(result);
      }
    }
    ...
    console.error('Wikimedia/Blob error:', error.message);
    return Response.json({ found: false });
  }
}
