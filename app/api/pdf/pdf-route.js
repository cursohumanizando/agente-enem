    ...
    return `
      <div class="questao ${i > 0 ? 'page-break' : ''}">
        ...
        ${q.recursoVisual && q.recursoVisual.descricao ? (() => {
          ...
          return `<div class="recurso-visual">
            <div class="recurso-icone">${
              q.recursoVisual.tipo === 'mapa' ? '🗺' :
              q.recursoVisual.tipo === 'gráfico' ? '📊' :
              q.recursoVisual.tipo === 'tabela' ? '📋' :
              q.recursoVisual.tipo === 'charge' ? '🎨' :
              q.recursoVisual.tipo === 'fotografia' ? '📷' : '📌'
            } ${(q.recursoVisual.tipo || '').toUpperCase()}</div>
            ${wimg ? `<img src="${wimg.dataUrl}" style="max-width:100%;max-height:320px;display:block;margin:10px auto;border-radius:6px;" alt="${q.recursoVisual.descricao}">
            <div class="recurso-fonte">🔍 Wikimedia Commons — ${q.recursoVisual.descricao}</div>` :
            `<div class="recurso-desc">${q.recursoVisual.descricao}</div>`}
            ${q.recursoVisual.fonteRecurso ? `<div class="recurso-fonte">📎 Fonte sugerida: ${q.recursoVisual.fonteRecurso}</div>` : ''}
          </div>`;
        })() : ''}
