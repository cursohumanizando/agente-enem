...
function buildHTML(questoes, wikiImgs = {}) {
  const questoesHTML = questoes.map((q, i) => {
    ...
    // Recurso visual: só mostra se tiver imagem real — sem placeholder
    const recursoHTML = (() => {
      if (!q.recursoVisual?.descricao) return '';
      const wimg = wikiImgs[q.numero];
      if (!wimg) return ''; // sem imagem = não mostra nada
      return `
        <div class="recurso-visual">
          <div class="recurso-label">${
            q.recursoVisual.tipo === 'mapa' ? '🗺 MAPA' :
            q.recursoVisual.tipo === 'gráfico' ? '📊 GRÁFICO' :
            q.recursoVisual.tipo === 'tabela' ? '📋 TABELA' :
            q.recursoVisual.tipo === 'charge' ? '🎨 CHARGE' :
            q.recursoVisual.tipo === 'fotografia' ? '📷 FOTOGRAFIA' : '📌 RECURSO VISUAL'
          }</div>
          <img src="${wimg.dataUrl || (wimg.imageUrl ? `/api/img?url=${encodeURIComponent(wimg.imageUrl)}` : '')}" alt="recurso visual" class="recurso-img">
          <div class="recurso-fonte">Fonte: Wikimedia Commons</div>
        </div>
      `;
    })();
    ...
    return `
      <div class="questao ${i > 0 ? 'page-break' : ''}">
        ...
        ${q.textoBase ? `<div class="texto-base">${q.textoBase.replace(/\n/g, '<br>')}</div>` : ''}
        ${q.fonte ? `<div class="fonte">${q.fonte}</div>` : ''}
        ${recursoHTML}
        ...
        <div class="gabarito-comentado">
          ...
          ${q.opcoes.filter(op => op.correta).map(op => `
            <div class="gab-correta">
              ...
              <div class="gab-body">
                <div class="gab-tag-ok">✓ Alternativa Correta</div>
          ...
          ${q.opcoes.filter(op => !op.correta).map(op => `
            <div class="gab-distrator">
              ...
              <div class="gab-body">
                <div class="gab-tag-err">✗ Distrator</div>
                <div class="gab-texto">${op.explicacao || 'Alternativa incorreta.'}</div>
              </div>
            </div>
          `).join('')}

          ${q.habilidade ? `<div class="gab-habilidade">🎯 ${q.habilidade}</div>` : ''}
        </div>
      </div>
    `;
  }).join('');
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .fonte { font-size: 11px; color: #9090b0; text-align: right; margin-bottom: 14px; font-family: monospace; }

  /* Recurso visual — imagem grande, sem texto descritivo */
  .recurso-visual {
    border: 1.5px solid #d4c060;
    background: #fdfbf0;
    border-radius: 8px;
    padding: 14px;
    margin-bottom: 16px;
  ...
  .opcoes { display: flex; flex-direction: column; gap: 8px; margin-bottom: 0; }
